...
- [ ] Continue Phase 1 research against the remaining source families in §6 below / `Project ATLAS-WebSite.md` §63 — eighteen source-family investigations across four batches so far (`docs/research/PHASE1-SOURCE-VERIFICATION.md`), including real negative findings (TxDOT camera data not found via its ArcGIS portal; Data.gov's classic CKAN API appears retired) recorded plainly rather than omitted. Not yet touched: the majority of individual state 511/DOT systems beyond WA, Socrata/CKAN discovery beyond Data.gov specifically, and the environmental/emergency federal source families (USFS, BLM, USACE, BOR, NPS webcams).
...
- [ ] **OpenEye re-investigation requires real browser automation, not more URL guessing.** This session's `WebFetch`-based documentation summary named specific API endpoints that all returned HTTP 404 on direct testing — the summary was very likely synthesized rather than read from real content, since `openeye.cam/docs` is a client-rendered app with no documentation text in its raw HTML. Per `MacEvil.md` §62, a controlled browser session (render the JS, capture real network requests) is the correct next step, not further guessing. Recorded as `FAILED_VALIDATION`, not silently dropped (`docs/research/PHASE1-SOURCE-VERIFICATION.md` §7).
...
- [ ] FEMA/IPAWS live CAP alert feeds (distinct from the OpenFEMA disaster-declarations dataset verified this session) remain unverified — good next Phase 1 candidate.
