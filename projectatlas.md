...
- [ ] Continue Phase 1 research against the remaining source families in §6 below / `Project ATLAS-WebSite.md` §63 — thirteen sources verified so far across three batches (`docs/research/PHASE1-SOURCE-VERIFICATION.md`). Not yet touched: GitHub camera-repository discovery as a standalone search methodology, nationwide 511/DOT discovery beyond WSDOT, Data.gov/Socrata/CKAN discovery, and refined OpenStreetMap querying (the first attempt hit a real Overpass API timeout — see below).
- [x] **Two stale duplicate root files, named explicitly — proposed resolution, not yet executed (needs operator sign-off):**
  - `ChatGPT MacEvil to ProjectATLAS handoff.md` (root, 1,592 lines) duplicates `docs/INDOT-handoff-primary.md` in full (confirmed by diff — the `docs/` copy is identical plus one added cross-reference note). **Proposed: delete the root file outright.** Unlike `MacEvil.md` §27/§79 (where both sections remain part of one document a reader might land on either half of, so a pointer helps), this is two entire separate files where `docs/INDOT-handoff-primary.md` is already the canonical, catalogued home per this file's own document map — a root-level stub adds no navigational value.
  - `video_api_captures.md` (root, 17,000 bytes) duplicates `docs/INDOT-handoff-supplemental.md`'s content. **Proposed: delete the root file outright**, for the same reason as above, plus an additional reason specific to this one — its filename actively misleads: it reads as if it *is* the lost original `video_api_captures.json` capture (confirmed `docs/ATLAS-ASSESSMENT.md` §1: it is not; it's a markdown copy of the recovered-memory document). Leaving a file with this name on disk risks a future session or the operator mistaking it for recovered original evidence.
  - Neither deletion has been executed. Waiting for explicit operator sign-off in a future turn, per that turn's instructions.
