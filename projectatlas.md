...
- [ ] Continue Phase 1 research against the remaining source families in §6 below / `Project ATLAS-WebSite.md` §63 — thirty-one source-family investigations across six batches so far, tracked authoritatively in a **Coverage Matrix** at the top of `docs/research/PHASE1-SOURCE-VERIFICATION.md` (including a state-by-state tracker for the two 50-state sweeps — nationwide 511/DOT now at 6/50 states via a proven, nationally-scaling ArcGIS/Socrata discovery methodology; OSM camera-tag discovery still at 1/50). Check that matrix first before assuming a family is untouched. Only emergency response data (fire/EMS/police/CAD feeds) remains entirely UNSTARTED — US federal land-management sources (NPS/BLM/USFS/USACE/BOR) were touched this session (`docs/research/PHASE1-SOURCE-VERIFICATION.md` §26).
- [x] **ArcGIS REST discovery methodology confirmed to scale nationally (2026-08-24).** ArcGIS Online's own public search API found 758 real "traffic camera" matches nationwide, 202 distinct owners in one 300-result sample, ≥15 identifiable US states. Six states now independently verified end-to-end with real counts: WA (1,533), OR (1,164), IA (1,251), MD (552), GA (7,083 — unusually high, magnitude unexplained), UT (32). Deprioritize blind per-state URL guessing (the Colorado/TxDOT approach) in favor of this for the rest of the 511/DOT sweep (`docs/research/PHASE1-SOURCE-VERIFICATION.md` §24).
- [x] **Socrata's 76 catalog matches fully categorized, not just counted (2026-08-24).** 19 non-US, 11 US enforcement/violation datasets (different category — some may be relevant to the ALPR-location policy track separately), 6 federal research-video archives, 30 pure noise, and 10 candidate entries deduplicating to 6 real US live-camera datasets, all independently verified with real record counts: Austin TX (1,005), Baton Rouge LA (118), New Orleans LA (103, specifically school-zone cameras), Honolulu HI (253), Maryland (451, cross-confirms the ArcGIS CHART finding above with a different count from a different endpoint — not yet reconciled), Seattle WA (real but not directly record-countable, a GIS-feature-class pointer) (`docs/research/PHASE1-SOURCE-VERIFICATION.md` §25).
- [x] **Federal land-management agencies (NPS/BLM/USFS/USACE/BOR) researched (2026-08-24).** NPS has a real, confirmed webcams API endpoint (`developer.nps.gov/api/v1/webcams`, confirmed via a structured `API_KEY_MISSING` error, not a 404) — needs an operator-supplied free key before further verification. The other four have real negative findings, each with real context: BLM's own verified recreation ArcGIS service has no camera layer; USFS wildfire cameras are actually university-operated (ALERTWildfire/ALERTCalifornia), not USFS's own — a real provenance correction if this family is ever pursued; USACE has a real water-data API but no camera API, and district webcam pages are fragmented across 18+ districts; BOR has a real Shasta Dam webcam but its RISE data API is water-data-only (`docs/research/PHASE1-SOURCE-VERIFICATION.md` §26).
