...
- [ ] Continue Phase 1 research against the remaining ~20+ source families in §6 below / `Project ATLAS-WebSite.md` §63 — eight sources verified so far across two batches (`docs/research/PHASE1-SOURCE-VERIFICATION.md`): NWS alerts, USGS earthquakes, INDOT WZDx, OpenTrafficCamMap, FEMA/OpenFEMA disaster declarations, WSDOT ArcGIS traffic cameras (VERIFIED/PARTIALLY_VERIFIED); Open511 (DEFERRED — no US implementation exists) and OpenEye (FAILED_VALIDATION — see below) are resolved-for-now, not open research items.
