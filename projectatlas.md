...
- [ ] Continue Phase 1 research against the remaining source families in §6 below / `Project ATLAS-WebSite.md` §63 — thirty-eight source-family investigations across eight batches so far, tracked authoritatively in a **Coverage Matrix** at the top of `docs/research/PHASE1-SOURCE-VERIFICATION.md` (state-by-state tracker: nationwide 511/DOT at 9/50 states, OSM camera-tag discovery at 1/50). Check that matrix first before assuming a family is untouched. Only emergency response data (fire/EMS/police/CAD feeds) remains entirely UNSTARTED.
- [x] **ArcGIS REST discovery: full 758-match set mined and status-filter-audited, 9 states verified (2026-08-24).** WA (1,533), OR (1,164), IA (1,251), MD (552), GA (3,676 active/7,083 total), UT (1,188 active/1,437 total — the CCTV layer; the originally-reported Live_View_Cameras layer was found to not be a camera dataset at all), FL (4,057), MO (871, new), VA (1,231 active/1,293 total, new). Every state audited for the same active/retired-record issue Georgia had — 5 confirmed already clean (OR/WA/IA/MD/FL), 2 corrected (Utah, twice). One real false positive caught (Pennsylvania — the matched item is a billboard-permit database, not cameras) and one real dead-link finding (North Carolina — an own-domain catalog entry 404s). Deprioritize blind per-state URL guessing in favor of this for the rest of the 511/DOT sweep (`docs/research/PHASE1-SOURCE-VERIFICATION.md` §24, §28, §30, §31).
