...
- [ ] Continue Phase 1 research against the remaining source families in §6 below / `Project ATLAS-WebSite.md` §63 — twenty-three source-family investigations across five batches so far, now tracked authoritatively in a **Coverage Matrix** at the top of `docs/research/PHASE1-SOURCE-VERIFICATION.md` (including a state-by-state tracker for the two 50-state sweeps — nationwide 511/DOT and OSM camera-tag discovery — currently at 3/50 and 1/50 states respectively). Check that matrix first before assuming a family is untouched. Still entirely UNSTARTED: US federal land-management camera sources (NPS/BLM/USFS/USACE/BOR), and emergency response data (fire/EMS/police/CAD feeds).
