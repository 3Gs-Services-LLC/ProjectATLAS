# SUPPLEMENTAL HANDOFF — Recovered Evidence From `video_api_captures.json`

*(Preserved verbatim in the project repository on 2026-08-23. Condensed, decision-relevant extract lives in `projectatlas.md` §5B — read that first; come here for full methodology/evidence detail. This document supersedes nothing in `docs/INDOT-handoff-primary.md`; it supplements it.)*
...
**Confirmed 2026-08-23: this loss is permanent — the file was searched for and not found. Nothing below should be treated as directly replayable (no exact request headers, query strings, or full JSON payloads survive). Where an exact protocol detail is required, it must be re-captured from the live system, not reconstructed from this memory.**
...
**Note (2026-08-23): `inhub.carsprogram.org` — the CARS-Hub XML feed portal independently verified this session (see `sources/indot-cars-hub/`) — is a fourth, distinct subdomain of `carsprogram.org`, serving the separate XML-feed integration path (§5A of `projectatlas.md`), not the GraphQL/HLS path described in this document. Do not conflate the two.**
...
**This time, save the resulting capture file into the project repository itself (e.g. `docs/evidence/`) rather than a desktop path outside the repo, so it cannot be lost the way `video_api_captures.json` was.**
...
**Policy note added 2026-08-23 (see `projectatlas.md` §5B and §8): this GraphQL endpoint was discovered by observing the public website's own client traffic, not from published API documentation. Before any production use beyond personal/internal development — and certainly before public redistribution — find and read 511in.org's terms of use, and classify this source's access policy explicitly rather than assuming it carries the same open status as the documented CARS-Hub XML feeds.**
