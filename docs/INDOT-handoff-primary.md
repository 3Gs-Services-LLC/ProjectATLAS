...
*(Preserved verbatim in the project repository on 2026-08-23. Originally authored as a ChatGPT→Claude Code technical handoff. Condensed, decision-relevant extracts of this document live in `projectatlas.md` §5 — read that first; come here for full narrative/methodology detail.)*
...
**Note (added 2026-08-23, per projectatlas.md §2):** Rule 7 above ("private/local viewer, not a clone of the public 511 website") described that sub-project's original narrower objective. It has since been superseded at the product level — ProjectATLAS's current, agreed product identity is a public, USA-only website, with an optional login/personalization layer layered on top rather than the core deliverable. See `projectatlas.md` §2 for the current, authoritative product definition. Everything else in this document — the verified INDOT/Castle Rock technical findings — remains accurate and valuable regardless of that product-level change.
...
# 2. CURRENT PROJECT OBJECTIVE (historical — see note above)

The project began as an Indiana Traffic Viewer and evolved into a private/local desktop application.

Current conceptual product (at the time this document was authored):
...
**Status as of 2026-08-23: the exact on-disk location of this Electron project has not yet been re-identified in the current session. Treat all of §3 and §4 below as historical technical memory to guide re-discovery, not as a confirmed, currently-existing codebase.**
...
**Note (2026-08-23): A live copy of this same CCTV XML feed family was independently re-fetched this session from `https://inhub.carsprogram.org/data/cctv.xml` (see `sources/indot-cars-hub/`). The exact ~740 count has not yet been recounted against that fresh download — treat the count above as historical until an adapter parses the new copy.**
...
**Status (2026-08-23): confirmed permanently lost. See `docs/INDOT-handoff-supplemental.md` for the recovered-memory extraction of its findings, and §5B of `projectatlas.md` for the condensed, decision-relevant version.**
...
# 28. CURRENT PRODUCT DIRECTION (historical — see note under §0/§2)
...
**Note (2026-08-23): this UI direction was explicitly carried forward as the "merge the concepts" resolution in projectatlas.md §2 — it should be layered onto the public ATLAS website as an enhancement, not treated as requiring a private/login-gated desktop app.**
...
**Note (2026-08-23): FEU-w was additionally discovered live this session (see `sources/indot-cars-hub/`); feu-m.xml and feu-w.xml both returned HTTP 401 Unauthorized on direct fetch, so treat both as REQUIRES_CREDENTIALS, not as confirmed-open feeds.**
...
**Status (2026-08-23): none of these artifacts have been re-located in the current environment. `video_api_captures.json` is confirmed permanently lost. The others' status is unknown pending discovery of the Electron project folder.**
